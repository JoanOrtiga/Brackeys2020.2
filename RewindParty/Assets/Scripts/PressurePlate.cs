...
public class PressurePlate : EnergyActivators
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if(canPlayerActivate && tag == "Player" || (canEnemyActivate && tag == "Enemy" && Vector2.Distance(collision.transform.position, transform.position) < 0.4f) || canBoxActivate && tag == "MoveableBox")
        {
            imActive = true;
